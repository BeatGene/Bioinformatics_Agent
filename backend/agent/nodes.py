...
def planner_node(state: ResearchState) -> dict:
    ...
    try:
        ...
        log.append(f"[Planner] LLM 调用失败，使用默认计划: {e}")
    ...
    log.append(f"[Planner] 完成。计划: {plan_summary}")
    for i, step in enumerate(steps, 1):
        log.append(f"  Step {i}: {step}")
...
def search_node(state: ResearchState) -> dict:
    ...
    if user_selected:
        ...
        if selected:
            return {
                ...
                "execution_log": [
                    f"[Search Worker] 用户手动选择 {len(selected)} 篇文献",
    ...
    try:
        ...
        return {
            "search_results": [],
            "selected_papers": [],
            "execution_log": logs + [f"[Search Worker] 检索失败: {e}"],
            "errors": [f"PubMed 检索失败: {e}"],
            "current_step": "检索失败",
            "next_node": "report",  # 直接跳到报告，生成错误报告
        }
    ...
    return {
        ...
        "user_approved_search": False,  # 重置，等待用户审核
        "user_feedback": "",            # 清除上次反馈
        "user_adjusted_query": "",      # 清除调整的查询
...
def review_search_node(state: ResearchState) -> dict:
    """检索审核节点——这是一个 HITL 中断点，前端在此处暂停并等待用户决定。
    如果用户批准，则 user_approved_search=True → 继续到 parse。
    如果用户要求重搜，则 user_feedback 和 user_adjusted_query 已被前端设置 → 返回 search。
    """
    user_feedback = state.get("user_feedback", "")
    user_adjusted = state.get("user_adjusted_query", "")
    user_selected = state.get("user_selected_ids", [])
    ...
    if user_feedback or user_adjusted:
        ...
        return {
            ...
            "next_node": "search",
            "execution_log": [f"[HITL] 用户要求调整检索: {user_feedback or user_adjusted}"],
    ...
    if user_selected:
        ...
        return {
            ...
            "next_node": "parse",
            "execution_log": [f"[HITL] 用户手动选择 {len(user_selected)} 篇文献，进入解析"],
...
def compare_node(state: ResearchState) -> dict:
    ...
    try:
        ...
        return {
            ...
            "execution_log": logs + [f"[Compare Worker] 失败: {e}"],
